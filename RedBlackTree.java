...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    /**
     * Resolves any Red Black Tree violations that may have been introduced
     * by inserting a new node in the tree.
     * 
    ...
    protected void enforceRBTreePropertiesAfterInsert(Node<T> node) {
        ...
        // Satisfy the root property
        if (node == this.root) {
            node.blackHeight = 1;
            return;
        }
        // Don't work on black nodes, move to parent
        if (node.blackHeight != 0) {
            enforceRBTreePropertiesAfterInsert(node.context[0]);
            return;
        }
        // Don't work on nodes whose parents are black
        if (node.context[0].blackHeight != 0) {
            enforceRBTreePropertiesAfterInsert(node.context[0].context[0]);
            return;
        }
        // Now we only have red nodes whose parents are also red (Red Property)
        Node<T> uncle, parent, grandparent;
        ...
        if (uncle == null || uncle.blackHeight != 0) {
            // A line is formed (Both parent and node are right children, or are left
            // children)
            ...
            // A triangle is formed, parent and node are not both right children or both
            // left children
            rotate(node, parent);
            enforceRBTreePropertiesAfterInsert(parent);
            return;
    ...
    private void recolor(Node<T> node) {
        node.blackHeight = node.blackHeight == 0 ? 1 : 0;
    }
    ...
    private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        if (child != parent.context[1] && child != parent.context[2]) {
            throw new IllegalArgumentException("Child is not related to the parent node.");
        }
    ...
    /**
     * This method performs a level order traversal of the tree. The string
     * representations of each color (blackHeight) value
     * within this tree are assembled into a comma separated string within
     * brackets (similar to many implementations of java.util.Collection).
     * 
     * @return string containing the blackHeights of this tree in level order
     */
    public String toLevelOrderColorString() {
        StringBuffer sb = new StringBuffer();
        sb.append("[ ");
        if (this.root != null) {
            LinkedList<Node<T>> q = new LinkedList<>();
            ...
            while (!q.isEmpty()) {
                ...
                if (next.context[2] != null)
                    q.add(next.context[2]);
                sb.append(next.blackHeight.toString());
                if (!q.isEmpty())
                    sb.append(", ");
            }
        }
    ...
    /**
     * This method tests the recoloring of the tree.
     * The nodes here are inserted such that no rotation happens
     */
    @Test
    void testRecoloring() {
        ...
        String levelColorOrder = tree.toLevelOrderColorString();

        assertEquals("[ 8, 5, 15, 12, 19, 9, 13, 23 ]", levelOrder);
        assertEquals("[ 1, 1, 0, 1, 1, 0, 0, 0 ]", levelColorOrder);
    }

    /**
     * This method tests insertion of three nodes to a tree and asserts that the
     * tree is rotated correctly given ther is no uncle and that the nodes are also
     * colored correctly
     */
    @Test
    void testSimpleRotation() {
        ...
        assertEquals("[ 5, 1, 15 ]", levelOrder);
        assertEquals("[ 1, 0, 0 ]", levelColorOrder);
    }

    /**
     * In this test, the Red property is violated way below the tree, the tree
     * should be rotated, rebalanced, and recolored appropriately multiple times
     */
    @Test
    void testRecursiveRotation() {
        RedBlackTree<Integer> tree = new RedBlackTree<>();
        assertDoesNotThrow(() -> {
            tree.insert(8);
            tree.insert(5);
        ...
        assertEquals("[ 12, 8, 15, 5, 9, 13, 19, 10, 23 ]", levelOrder);
        assertEquals("[ 1, 0, 0, 1, 1, 1, 1, 0, 0 ]", levelColorOrder);
    }
}
